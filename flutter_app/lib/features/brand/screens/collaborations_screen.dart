...
class BrandCollaborationsScreen extends StatefulWidget {
  const BrandCollaborationsScreen({super.key});

  @override
  State<BrandCollaborationsScreen> createState() =>
      _BrandCollaborationsScreenState();
}
...
class _BrandCollaborationsScreenState extends State<BrandCollaborationsScreen> {
  ...
  Widget _buildBodyState() {
    if (_errorMessage != null && _collaborations.isEmpty) {
      return AppRefreshableState(
        onRefresh: () => _loadCollaborations(showLoading: false),
        child: AppErrorState(
          message: _errorMessage!,
          onRetry: () => _loadCollaborations(),
        ),
      );
    }
  ...
  Widget _buildCollabCard(Collaboration collab) {
    final profile = collab.creator?.creatorProfile;
    final creatorName = profile?.fullName ?? 'Créateur';
    final creatorImage = profile?.profilePicture;
    final title = collab.announcement?.title ?? 'Collaboration #${collab.id}';
    final statusColor = _getStatusColor(collab.status);

    return GestureDetector(
      onTap: () async {
        await context.push('/brand/collaboration/${collab.id}');
        _loadCollaborations(showLoading: false);
      },
      child: Container(
        ...
        child: Row(
          children: [
            CircleAvatar(
              ...
              backgroundImage: creatorImage != null
                  ? CachedNetworkImageProvider(creatorImage)
                  : null,
            ...
            if (collab.unreadCount != null && collab.unreadCount! > 0)
              Container(
